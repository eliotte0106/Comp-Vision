...
def compute_image_gradients(image_bw: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    r, c = SOBEL_X_KERNEL.shape[0],SOBEL_X_KERNEL.shape[1]
    padded= np.pad(image_bw, ((r//2, r//2), (c//2, c//2)), 'constant', constant_values=[0])
    Ix = np.zeros(image_bw.shape).astype(np.float32)
    Iy = np.zeros(image_bw.shape).astype(np.float32)
    for i in range(image_bw.shape[0]):
      for j in range(image_bw.shape[1]):
          filter_range = padded[i:i+r, j:j+c]
          Ix[i][j] = np.sum(filter_range * SOBEL_X_KERNEL)
          Iy[i][j] = np.sum(filter_range * SOBEL_Y_KERNEL)
...
def nms_maxpool_pytorch(
    ...
    med=torch.median(R)
    R[R <= med] = 0.0
    maxpool = nn.MaxPool2d((ksize,ksize), stride=1, padding=(ksize//2,ksize//2), return_indices=True)
    maxed, indices = maxpool(R)
    x_1 = torch.zeros(R.shape)
    x_2 = torch.ones(R.shape)
    binarized_to = torch.where(R >= maxed, x_2, x_1)
    result = torch.mul(R, binarized_to)
    ...
    for i in range(k):
        confidences[i] = np.max(result)
        h = np.where(result == np.max(result))[0][0]
        w = np.where(result == np.max(result))[1][0]
        result[h,w] = -1
        x[i] = w
        y[i] = h
