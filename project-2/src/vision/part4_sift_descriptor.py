...
def get_gradient_histogram_vec_from_patch(
    ...
    for i in range(0, 4):
        for j in range(0, 4):
            c = window_orientations[4 * i:4 * i + 4, 4 * j:4 * j + 4]
            w = window_magnitudes[4 * i:4 * i + 4, 4 * j:4 * j + 4]
            hist = np.histogram(c, bins=bins, range=r, weights=w)[0]
...
def get_feat_vec(
    ...
    r = int(r)
    c = int(c)
    mag = magnitudes[r - (feature_width // 2 - 1): r + (feature_width // 2) + 1, c - (feature_width // 2 - 1): c + (feature_width // 2) + 1]
    ori = orientations[r - (feature_width // 2 - 1): r + (feature_width // 2) + 1, c - (feature_width // 2 - 1): c + (feature_width // 2) + 1]
    his = get_gradient_histogram_vec_from_patch(window_magnitudes = mag, window_orientations = ori)
    fv = his / np.linalg.norm(his)
    fv = np.sqrt(fv)
    # r = feature_width // 2
    # window_magnitudes = magnitudes[int(c-r+1): int(c+r+1), 
    #                                int(r-r+1):int(r+r+1)]
    # window_orientations = orientations[int(c-r+1):int(c+r+1),
    #                                   int(r-r+1):int(r+r+1)]
    # c = int(feature_width/4)
    # c_point = [0, c, c*2, c*3]
    # wgh = []
    # bin=8
    # bin_shape = np.array([-8,-6,-4,-2,0,2,4,6,8])*np.pi/bin
    # for i in c_point:
    #     for j in c_point:
    #         mag = window_magnitudes[i:i+c,j:j+c].flatten()
    #         ori = window_orientations[i:i+c,j:j+c].flatten()
    #         temp = np.histogram(ori,bin_shape,weights = mag)
    #         wgh.extend(temp[0].tolist()) 
    # wgh = np.array(wgh).reshape(c*c*8,1)
    # wrong ( does not work with this code)
    # if np.linalg.norm(wgh) == 0:
    #     wgh = np.zeros((0,1))
    # else:
    # wgh /= np.linalg.norm(wgh)
    # fv = np.sqrt(wgh)
    # fv = fv**0.9
    #############################################################################
    #                             END OF YOUR CODE                              #
...
def get_SIFT_descriptors(
    ...
    mag, ori = get_magnitudes_and_orientations(Ix=Ix, Iy=Iy)
    fvs = np.zeros((X.shape[0], 128))
    for i, coordinates in enumerate(zip(Y, X)):
        fvs[i, :] = get_feat_vec(coordinates[1], coordinates[0], magnitudes = mag, orientations = ori)[:, 0]
