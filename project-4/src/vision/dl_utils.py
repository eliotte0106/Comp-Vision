...
def compute_accuracy(logits: torch.Tensor, labels: torch.Tensor) -> float:
    ...
    correct_pred = pred.eq(labels).sum()*1.0
    batch_accuracy = correct_pred.item()/batch_size
...
def compute_loss(
    ...
    if is_normalize: loss /= float(list(model_output.shape)[0])
    # raise NotImplementedError(
...
def compute_multilabel_accuracy(logits: torch.Tensor, labels: torch.Tensor) -> float:
    ...
    pred_num = torch.where(logits>0.5,1,0)

    batch_accuracy = float(torch.sum(pred_num == labels)) / float(torch.numel(labels))
