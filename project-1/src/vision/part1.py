...
def create_Gaussian_kernel_1D(ksize: int, sigma: int) -> np.ndarray:
    ...
    """
    # x1 = np.ones(ksize)
    # r = np.linspace(-int(ksize/2), int(ksize/2), ksize)
    # x1 = [1 / (sigma * np.sqrt(2*np.pi)) * np.exp(-x**2/(2*sigma**2)) for x in r]
    # x1 = np.asarray(x1)
    # kernel = np.outer(x1,1)
    # kernel = kernel/np.sum(kernel)
    mean = np.floor(ksize/2)
    x = np. arange(-mean, mean + 1)
...
def create_Gaussian_kernel_2D(cutoff_frequency: int) -> np.ndarray:
    ...
    gass_1d = create_Gaussian_kernel_1D(k, sigma)
    kernel = np.outer(gass_1d, gass_1d)
...
def my_conv2d_numpy(image: np.ndarray, filter: np.ndarray) -> np.ndarray:
    ...
    row, col = filter.shape[0], filter.shape[1]
    x = row // 2
    y = col // 2
    pad_image = np.pad(image, ((x, x), (y, y), (0, 0)), 'constant', constant_values = 0)
    filtered_image = np.zeros(image.shape)
    for i in range(image.shape[0]):
      for j in range(image.shape[1]):
        for k in range(image.shape[2]):
          filter_range = pad_image[i:i+row, j:j+col, k]
          filtered_image[i][j][k] = np.sum(filter_range * filter)
