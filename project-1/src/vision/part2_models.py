...
from src.vision.part1 import create_Gaussian_kernel_2D
...
class HybridImageModel(nn.Module):
    ...
    def get_kernel(self, cutoff_frequency: int) -> torch.Tensor:
        ...
        gauss_kernel = create_Gaussian_kernel_2D(cutoff_frequency)
        k = gauss_kernel.shape
        kernel = np.reshape(gauss_kernel,(1,1,k[0],k[1]))
        c = self.n_channels
    ...
    def low_pass(self, x: torch.Tensor, kernel: torch.Tensor):
        ...
        row = kernel.shape[2]
        col = kernel.shape[3]
        filtered_image = F.conv2d(x,kernel,padding=(row//2,col//2),groups = self.n_channels)
    ...
    def forward(
        ...
        hybrid_image = torch.clamp(high_frequencies+low_frequencies,0,1)
        ### END OF STUDENT CODE ####
        ############################
